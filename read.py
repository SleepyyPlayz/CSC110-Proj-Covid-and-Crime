...
def get_police_data(data: list[EmergencyCall], location: str, year: int, category: str) -> \
    ...
    """
    crimes = analyze.filter_just_crimes(data)
    crimes_in_location = analyze.filter_crimes_by_location(crimes, location, year)
    crimes_category, crimes_opp_category = analyze.filter_crimes_by_type(crimes_in_location, category)
...
def get_covid_data(data: list[CovidData], location: str, year: int) -> dict[str, list]:
    ...
    Preconditions:
      ...
      - year >= 0
    """
    covid_in_location = analyze.get_monthly_cases(data, year, location)
